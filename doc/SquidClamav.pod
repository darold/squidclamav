...
bad limitations and broke many sites using cookies and post HTTP method. The
reason is that older squidclamav releases (< 6.0) had to request the URL given 
by Squid through libcurl to download the file and scan it. But this URL doesn't
...
SquidClamav v6 bypasses theses limitations by scanning only the HTTP stream sent
by Squid through the ICAP server. It doesn't make HTTP requests itself so this is
...
SquidClamav v6 is still faster than any other HTTP antivirus and can handle 
...
for testing your URL. All debug information will now go to the c-icap logfile.
...
Here the bypass is set to 1, that means that in case of ICAP problems squid will
...
load a template up from disk and send this back to the user. By default this
...
is packed inside a CVD file and distributed through our mirror network as
