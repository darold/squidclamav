...
SquidClamav v6 is an antivirus for the Squid proxy based on the ICAP protocol
...
bad limitations and breaks many sites using cookies and post HTTP method. The
reason is that older squidclamav release (< 6.0) had to request through libcurl
the URL given by Squid to download the file and scan it. But this URL doesn't
...
SquidClamav v6 bypass theses limitations by only scanning the HTTP stream sent
by Squid through the ICAP server. It don't make HTTP request itself so this is
...
ICAP feature allowing "on stream" scanning. It is now built as a c-icap server
...
for testing your URL. All debug information will now goes to the c-icap logfile.
...
Here the bypass is set to 1, that mean that in case of ICAP problem squid will
...
What that configuration directives do? It enables the ICAP client into Squid and
tells Squid to send the logged username and client's Ip to the ICAP server. It
also enable preview for faster SquidClamav work. The last four lines defined how
to call the ICAP server. Here we call the squidclamav service on localhost and
port 1344 but it can be on any other host and port. The bypass parameter set
to 1 mean that Squid will continue without bothering about ICAP server or
...
(clamd_local). If your clamd daemon use INET socket or stay in a remote server,
you have to set the ip address and the port with clamd_ip and clamd_port.

If you use INET socket the 'clamd_local' directive must be commented else
...
can be establish SquidClamav will reuse this last "working" ip address first to
...
page. You can find in the SquidClamav distribution a set of Perl CGI scripts
...
SquidClamav allow you to chain the SquidGuard program to check the Url requested
against blocklists using the 'squidguard' directive. You just have to give the
path to the program.
...
=head2 Controlling SquidClamav behaviour
...
=head3 Control both chained program and virus scan
...
and virus scan at URL level. When the given pattern match the URL SquidClamav
...
chained program). When the URL match the regex pattern SquidClamav fallback to
...
squid-cache.org, the last one will exclude all PNG, GIF and JPEG image from
scanning.
...
because SquidClamav need to download the HTTP header for a file with a HEAD
request. Note that some sites do not answer to HEAD request so content type
...
The database is packed inside a CVD file and distributed through our mirror network as safebrowsing.cvd .
...
server with mutilple CPU will really help.
