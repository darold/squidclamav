...
SquidClamav v6 only scan the HTTP stream sent by Squid through the ICAP server.
...
SquidClamav v6 is faster than any other HTTP antivirus and can handle several
...
Enable this directive if you use SslBump in Squid to be able to scan https
connect request otherwise encrypted tunnels will not be scan at all by default.
Decrypting HTTPS tunnels without user consent or knowledge may violate ethical
norms and may be illegal in your jurisdiction. Make sure you understand what
you are doing and that your decision makers have enough information to make
wise choices. 
